...
class Flex4AxeHardware(QaxeHardware):
    def __init__(self, config):
        # Load settings from config
        self.config = config
...
class ZeroxAxe(Flex4AxeHardware):
    def __init__(self, config):
...
class BM1366Miner:
    ...
    def init(self):
        ...
        if self.miner == 'piaxe':
            ...
            self.hardware = ZeroxAxe(self.config[self.miner])
            self.asics = bm1366.BM1366()
        else:
        ...
        if rest_config is not None and rest_config.get("enabled", False):
            self.rest_api = rest.ASICFrequencyManager(rest_config, self.asics)
            self.rest_api.run()
